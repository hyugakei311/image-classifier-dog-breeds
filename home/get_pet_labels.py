...
def get_pet_labels(image_dir):
    ...
    for name in in_files:
        if name[0] == ".":
            ...
            pet_label = []
            words = name.split('.')[0].split('_')
            for word in words:
                if word.isalpha():
                    pet_label.append(word.strip().lower())
            
            if not name in results_dic:
                results_dic[name] = [(' ').join(pet_label)]
            else:
                print("** Warning: Duplicate files exist in directory:", name)
